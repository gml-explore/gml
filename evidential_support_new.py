...
class EvidentialSupport:
    ...
    def evidential_support(self,variable_set,update_feature_set):
        ...
        @return:
        '''
        self.data_matrix = evidential_support_by_regression.create_csr_matrix(self.variables, self.features)
        ...
        var_len = 0  # 统计函数化相关的变量个数
        fea_len = 0  # 统计函数化相关的特征个数
        ...
        if len(residuals) != 0 and len(Ns) != 0 and len(meanX) != 0 and len(variance) != 0:
            ...
            evidential_support_logit = csr_matrix((loges, (row, col)), shape=(var_len, fea_len))
            evidential_unsupport_logit = csr_matrix((logunes, (row, col)), shape=(var_len, fea_len))
            p_es = np.exp(np.array(evidential_support_logit.sum(axis=1)))
            p_unes = np.exp(np.array(evidential_unsupport_logit.sum(axis=1)))
            # approximate_weight = csr_matrix((espredict, (row, col)), shape=(var_len, fea_len))
        ...
        if type(variable_set) == list or type(variable_set) == set:
            ...
            for vid in variable_set:
                ...
                if 'binary_feature_evi' in self.variables[vid] or 'unary_feature_evi_prob' in self.variables[vid]:
