...
import sys
import argparse
from numbskull_extend.factorgraph import FactorGraph
from numbskull_extend.dataloading import *
...
arguments = [
    ...
    (('-i', '--n_inference_epoch'),
        {'metavar': 'NUM_INFERENCE_EPOCHS',
         ...
         'default': 0,
         'type': int,
         'help': 'number of numbskull_extend epochs'}),
    (('-s', '--stepsize', '-a', '--alpha'),
...
class NumbSkull(object):
    ...
    def loadFactorGraph(self, weight, variable, factor, fmap, domain_mask,
                        edges, alpha_bound,tau_bound,sample_list,wmap= None,wfactor =None,var_copies=1, weight_copies=1,
        ...
        fg = FactorGraph(weight, variable, factor, fmap,vmap, factor_index,
                         var_copies, weight_copies,
                         len(self.factorGraphs), self.nthreads,self.learning_method,
                         alpha_bound,tau_bound,sample_list,wmap,wfactor)
        self.factorGraphs.append(fg)
    ...
    def inference(self, fgID=0, out=False):
        """TODO."""
    ...
    def learning(self, fgID=0, out=False):
        """TODO."""
        burn_in = self.burn_in
